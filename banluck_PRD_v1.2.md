...
**Changelog from v1.1:**
- Phase 1 (Game Engine) marked complete — all 245 tests passing, all 14 edge cases verified
- Implementation decisions documented (ace greedy algorithm, five-card vs five-card payout, etc.)
- Project structure updated to reflect actual implemented layout
- Phase 1.1 (Baseline DP Solver) promoted to active next milestone
- v1.1 PRD superseded by this document
...
The dealer's selective reveal is a strategic tool. At totals of 16 or 17 (including soft 17), the dealer reveals 3+ card players first — these players may have busted — to collect those bets, then draws a card to try to improve before facing the stronger 2-card hands.
...
- Assumption: Dealer follows fixed strategy (reveal at 16/17, hit at 16/soft-17, stand at hard-17+)
- Output: Optimal player hit/stand chart for `(player_total, hand_size, dealer_upcard)`
...
**Dealer selective reveal at 16/17:**
...
# Edge Case 13: Dealer at 17 reveals 3+-card player BEFORE deciding to hit
...
- ✅ Formal game specification (PRD v1.0 → v1.2)
...
- ⏭️ Optimal player hit/stand chart (vs fixed dealer strategy)
...
**Week 3–4:** Baseline solver (player-only strategy, fixed dealer) ← ACTIVE
...
⏭️ **Active next step: Phase 1.1 — `baseline_dp.py` (backward induction solver)**

---

**This PRD (v1.2) is the canonical specification. The game engine is complete and frozen. Do not modify engine modules without a corresponding test update. Begin Phase 1.1 with `src/solvers/baseline_dp.py`.**
